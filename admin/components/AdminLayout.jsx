...
export default function AdminLayout({
  ...
  const isProcessReady = isCreating ? false : Boolean(selectedProcessId);

  return (
    <main className="admin-shell font-sans text-neutral-900 bg-background min-h-screen">
      <header className="mb-1 flex flex-col gap-1 border-b border-neutral-100 pb-3">
        <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-accent font-semibold">Where Do I Stand</p>
        <h1 className="text-2xl font-bold tracking-tight text-neutral-950">{title || "Admin Panel"}</h1>
      ...
      {(!hideNavigation || !hideActiveProcess) && (
        <nav className="step-nav" aria-label="Admin steps">
          {!hideNavigation && (
            <div className="step-nav-items">
              {steps.map((step) => {
                const isActive = router.pathname === step.href || (step.href === "/" && (router.pathname === "/create" || router.pathname === "/update"));
          ...
          {!hideActiveProcess && (
            <div className="process-chip flex items-center gap-2 py-1 px-2.5">
              ...
              <select
                ...
                disabled={!hydrated || isCreating}
              >
                <option value="">{isCreating ? "Creating New..." : "None Selected"}</option>
                {!isCreating && processes.map((processItem) => (
